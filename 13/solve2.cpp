...
public:
    ...
    // Function: fold
    // Description: Folds the paper
    // Parameters:
    //      string axis: The axis to fold from
    //      int value: The value to fold from
    // Return:
    //      None
    void fold(string axis, int value) {
        if (axis == "x") {
            this->fold_x_axis(value);
        } else if (axis == "y") {
            this->fold_y_axis(value);
        } else {
            assert(false);
        }
    }
...
int main(int argc, char *argv[]) {
    ...
    // Fold the paper
    for (int i = 0; i < input.fold_commands->size(); i++) {
        paper.fold(input.fold_commands->at(i).axis, input.fold_commands->at(i).value);
    }
