...
int main(int argc, char *argv[]) {
    ...
    BingoBoard * last_board = boards[0];

    // While len(boards) != 1 and there are numbers left
    while (boards.size() > 0 && numbers.size() > 0) {
        ...
        // Remove any board that has bingo from the global boards vector
        for (int i = 0; i < boards.size(); i++) {
            if (boards[i]->check_bingo()) {
                last_board = boards[i];
    ...
    // Assert last board has bingo
    if (last_board->check_bingo()) {
        cout << "Bingo! " << " called " << number_just_called << endl;
    } else {
        cout << "No bingo" << endl;
    }
    ...
    cout << "Solution: " << last_board->sum_unfilled_board() * number_just_called << endl;
